...
int establish_connection(const char *client_ip, const char *server_ip,
                         int port) {
...
int send_message_to_server(const char *client_ip, const char *server_ip,
...
int run_realtime_chat(const char *server_ip, int port) {
  ...
  while (1) {
    ...
    if (FD_ISSET(sock_fd, &read_fds)) {
      int opcode;
      int len = read_websocket_frame(sock_fd, recv_buffer,
                                     sizeof(recv_buffer) - 1, &opcode);
      if (len <= 0) {
...
//     {
//         int sock_fd = establish_connection(client_ip, "127.0.0.1",
//         SERVER_PORT); if (sock_fd >= 0) {
//             char *req = send_post_request((char *)"127.0.0.1", "/login",
//             (char *)username); send(sock_fd, req, strlen(req), 0); free(req);
//             char resp[512];
...
int sock_client_connect(sock_client_t *client, const char *username) {
  ...
  if (reg_fd >= 0) {
    ...
    if (connect(reg_fd, (struct sockaddr *)&srv_addr, sizeof(srv_addr)) == 0) {
      char *req =
          send_post_request(client->server_ip, "/login", (char *)username);
      if (req) {
...
char *sock_client_recv_message(sock_client_t *client, int timeout_ms) {
  ...
  if (activity > 0 && FD_ISSET(client->socket_fd, &read_fds)) {
    ...
    int opcode = 0;
    int len = read_websocket_frame(client->socket_fd, recv_buffer,
                                   sizeof(recv_buffer) - 1, &opcode);
    if (len > 0) {
      recv_buffer[len] = '\0';
