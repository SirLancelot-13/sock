...
int handle_ws_frame(int fd, const char *http_req){
    const char *key_hdr="Sec Web-Socket-Key: ";
    char *key_start=strstr(key_hdr, http_req);
    if (!key_start) perror("Failed to parse in the start of the http-request.\n");
    char *key_end=strstr(http_req, "\r\n");
    if (!key_end) perror("Failed to parse in the end of the http-request.\n");
    ...
    // Hash ts
    uint8_t hash[20];
    sha1_hash((const int8_t *)concatenate, strlen(concatenate), hash);
    // Convert to b64
    char b64[32];
